...
/**
 * Immutable request for the GPT provider.
 * <p>
 * Includes conversation fields ({@code storeConversation}, {@code prevResponseId}),
 * {@code countryIsoCode} (GPT-only), and company-search fields.
 */
@Getter
public final class GptSearchRequest implements JobSearchRequest {
  ...
  private GptSearchRequest(Builder builder) {
  ...
  public static final class Builder implements JobSearchRequest.ConversationBuilder {
    ...
    private Builder(GptSearchRequest source) {
    ...
    @Override
    public GptSearchRequest build() {
      return new GptSearchRequest(this);
    }
  }
}
